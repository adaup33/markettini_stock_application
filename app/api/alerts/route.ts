...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ 
      success: true, 
      data: items, 
      meta: { email: email ?? null, emailSource, emailDetail, page, limit, total } 
    });
  } catch (err) {
  ...
  try {
    ...
    try {
      ...
      await fetch(wsHost, { 
        ...
        body: JSON.stringify({ type: 'alerts:update', payload: { email, symbol, action: 'create' } }) 
      });
    } catch (e) { 
      console.error('alerts POST broadcast failed', e); 
    }

    return NextResponse.json({ 
      success: true, 
      data: created, 
      meta: { email: email ?? null, emailSource, emailDetail } 
    }, { status: 201 });
  } catch (err) {
